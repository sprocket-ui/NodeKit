...
export function useButton<T extends ElementType = typeof DEFAULT_BUTTON_TAG>(
	...
	if (elementType === HTMLElements.Button) {
		...
		additionalProps = {
			...
			'aria-disabled': !isDisabled || elementType === HTMLElements.Input ? undefined : isDisabled,
	...
	const activeStates: string[] = Object.entries(states)
		.filter(([, on]) => on)
		.map(([key]) => key);

	const dataAttributes: Record<string, string | undefined> = Object.fromEntries(
		Object.entries(states).map(([key, on]) => [`data-${key}`, on ? 'true' : undefined])
	);

	dataAttributes['data-sprocket-state'] =
		activeStates.length > 0 ? activeStates.join(' ') : undefined;
